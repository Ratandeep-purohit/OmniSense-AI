    ...
    def evaluate(
        ...
        *,
        ...
        if plan.context_id != snapshot.context.context_id:
            failures.append("plan/context identity mismatch")
        if not snapshot.is_fresh(max_age_seconds=max_age_seconds):
            failures.append("context is stale")
        if not plan.steps or len(plan.steps) > 10:
            failures.append("step count outside bounded execution limit")
