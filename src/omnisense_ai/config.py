...
def _parse_bool(value: str) -> bool:
    ...
    if normalized in _FALSE_VALUES:
        return False
    raise ValueError("OMNISENSE_CAPTURE_ENABLED must be a boolean value.")
...
def _parse_max_fps(value: str) -> float:
    try:
        ...
        raise ValueError("OMNISENSE_CAPTURE_MAX_FPS must be a number.") from exc

    if parsed <= 0 or parsed > _MAX_CAPTURE_FPS:
        raise ValueError(f"OMNISENSE_CAPTURE_MAX_FPS must be greater than 0 and at most {_MAX_CAPTURE_FPS:g}.")
    return parsed
...
def _parse_region(value: str) -> tuple[int, int, int, int] | None:
    ...
    if len(parts) != 4:
        raise ValueError("OMNISENSE_CAPTURE_REGION must use x,y,width,height format.")

    try:
        x, y, width, height = (int(part) for part in parts)
    except ValueError as exc:
        raise ValueError("OMNISENSE_CAPTURE_REGION values must be integers.") from exc

    if width <= 0 or height <= 0:
        raise ValueError("OMNISENSE_CAPTURE_REGION width and height must be positive.")
